// Standalone copy of the web version's radio language packs, decoupled from the
// React app's i18n/teams modules so it can run inside the WebAssembly build's
// page. Team names are passed through from the C++ engine as plain strings.
export type AppLanguage =
  | "ro" | "fr" | "en" | "es" | "pt" | "de" | "nb" | "it" | "ga" | "nl"
  | "hr" | "pl" | "tr" | "ru" | "uk" | "ar" | "hi" | "id" | "vi" | "th"
  | "ja" | "ko" | "zh-CN" | "zh-TW";

export const RADIO_LANGUAGES: AppLanguage[] = [
  "en", "fr", "es", "pt", "de", "nb", "it", "ga", "nl", "hr", "ro", "pl",
  "tr", "ru", "uk", "ar", "hi", "id", "vi", "th", "ja", "ko", "zh-CN", "zh-TW",
];
...
// Kept only as an example of localised country naming; unused in the wasm build
// because the C++ engine supplies the actual match team names.
...
// In the wasm build the C++ engine passes the real team name string, so this is
// a passthrough (the signature is kept for parity with the web version).
export function getTeamDisplayName(_language: AppLanguage, teamName: string): string {
  return teamName;
}
