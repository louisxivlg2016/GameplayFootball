...
/** The UI language the C++/page layer selected (default en, or ?lang=xx). */
function getCurrentLanguage(): AppLanguage {
  return uiLanguage;
}
/** Set by the page (URL ?lang=xx) or the in-game language switch. */
export function setRadioLanguage(lang: string): void {
  if ((RADIO_LANGUAGES as string[]).includes(lang)) uiLanguage = lang as AppLanguage;
}
function audioMuted(): boolean {
  return muted;
}
export function setRadioMuted(m: boolean): void {
  muted = m;
}
/** The C++ engine reports the two match team display names. */
export function setRadioTeams(home: string, away: string): void {
  teamNames = [home || "", away || ""];
}
function sharedAudioContext(): AudioContext | null {
  return null;
}
function sharedAudioOutput(): AudioNode | null {
  return null;
}
...
// compressor/gain "broadcast" chain), NOT a bare <audio> element. The element
// path tops out at volume 1.0 and proved unreliable here; WebAudio is louder and
// verifiably produces sound. (HTMLMediaElement is only preferred on mobile.)
const PREFER_HTML_AUDIO = false;

// Always-on tap on the radio output bus, so window.__radioPeak / audioPeak()
// report the true output level (used to confirm the commentator is audible).
...
function ensureRadioCtx(): AudioContext {
  if (!radioCtx) {
    ...
    if (shared.__gpfRadioOut) {
      ...
      captureAnalyser = radioCtx.createAnalyser();
      radioOut.connect(captureAnalyser);
...
function fetchGoalClipBytes(): Promise<ArrayBuffer | null> {
  goalClipBytes ??= fetch(GOAL_CLIP_URL)
...
// (warmupRadioVoice() is called by radioMain AFTER the URL language is applied,
// so the correct voice model loads first instead of English then a switch.)
...
export const teamName = (id: number, language: AppLanguage = currentLanguage()): string =>
  getTeamDisplayName(language, teamNames[id as 0 | 1] || (id === 0 ? "the home side" : "the away side"));
